...
namespace WebAPP.Controllers
{
    public class Login : Controller
    {
        ...
        // POST: Xử lý đăng nhập
        [HttpPost]
        public async Task<IActionResult> indexDangnhap(string TenDangNhap, string password)
        {
            using (var connection = _context.CreateConnection())
            {
                var sql = "SELECT * FROM TaiKhoanNhanVien WHERE TenDangNhap = @TenDangNhap";
                ...
                if (user != null)
                {
                    ...
                    if (user.PasswordHash.SequenceEqual(hashedInput))
                    {
                        ...
                        {
                            new Claim(ClaimTypes.NameIdentifier, user.ID_TaiKhoan.ToString()),
                            new Claim(ClaimTypes.Name, user.TenDangNhap),
                            new Claim(ClaimTypes.Role, user.QuyenHan ?? "User")
                        };
                        ...
                        return RedirectToAction("Soyeu", "Home");
                    }
                }
